...
export default function PartnerProfile() {
  ...
  const pt = PARTNER_TYPES[user?.role]
    ...
    <div className="min-h-screen">
      ...
      <div className="px-5 py-6 space-y-4">
        <div className="flex flex-col items-center py-4">
          <div className="w-20 h-20 rounded-full bg-mh-accent-light text-4xl flex items-center justify-center mb-3">{pt?.emoji || '🏪'}</div>
          <h2 className="text-lg font-bold text-mh-text">{user?.name}</h2>
          <span className="text-xs bg-mh-surface text-mh-text3 px-3 py-1 rounded-full mt-1 border border-mh-border">{pt?.label}</span>
        </div>
